/**
 * ReportarPasantiaModal — modal para que una universidad reporte una
 * PASANTÍA/vacante al admin (no a una persona). Escribe en `reportes` vía
 * reporteService.reportarPasantia con el esquema que lee el panel admin
 * (`tipo: 'pasantia'` + `vacante_id`). Theme-aware e i18n (AutoText).
 *
 * Uso:
 ...
 *     empresaNombre={v.nombre_empresa}
...
import { MOTIVOS_REPORTE_PASANTIA, reportarPasantia } from "../services/reporteService";
...
export default function ReportarPasantiaModal({
  visible,
  vacanteId,
  vacanteTitulo,
  empresaId,
  ...
  const enviar = async () => {
    ...
    if (!vacanteId) {
      setError("No se pudo identificar la pasantía.");
      return;
    }
    setSending(true);
    try {
      await reportarPasantia({
        empresaId,
        empresaNombre,
        vacanteId,
        vacanteTitulo,
      ...
      <View style={s.overlay}>
        <View style={s.card}>
          <View style={s.header}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={s.title}>Reportar pasantía</Text>
              {vacanteTitulo ? (
          ...
          {done ? (
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <View style={s.motivos}>
                {MOTIVOS_REPORTE_PASANTIA.map((m) => {
              ...
              <TextInput
                ...
                placeholder="Describe brevemente el problema con esta pasantía…"
