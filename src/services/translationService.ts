...
import { ADMIN_SEED_EN, AUTO_SEED_EN, BIENVENIDA_SEED_EN, CALIFICAR_PLATAFORMA_SEED_EN, CUPOS_SEED_EN, EMPRESA_TRABAJOS_SEED_EN, FEEDBACK_TARDE_SEED_EN, GESTION_SEED_EN, GRUPO_DETALLE_SEED_EN, INCIDENCIAS_SEED_EN, NOTIF_MODALES_SEED_EN, ONBOARDING_INICIAL_SEED_EN, PROGRESO_SEED_EN, RECLUTAMIENTO_SEED_EN, RESUMEN_HOME_SEED_EN, SIN_EMOJIS_SEED_EN, TOP_ESTUDIANTES_SEED_EN, UBICACION_SEED_EN } from "../i18n/autoSeed";
...
function seedStaticCache(): void {
  ...
  for (const es in FEEDBACK_TARDE_SEED_EN) {
    const en = FEEDBACK_TARDE_SEED_EN[es];
    if (en) mem[key("en", es)] = en;
  }
  for (const es in EMPRESA_TRABAJOS_SEED_EN) {
    const en = EMPRESA_TRABAJOS_SEED_EN[es];
    if (en) mem[key("en", es)] = en;
  }
  for (const es in GRUPO_DETALLE_SEED_EN) {
    const en = GRUPO_DETALLE_SEED_EN[es];
    if (en) mem[key("en", es)] = en;
  }
  for (const es in TOP_ESTUDIANTES_SEED_EN) {
    const en = TOP_ESTUDIANTES_SEED_EN[es];
    if (en) mem[key("en", es)] = en;
  }
  for (const es in BIENVENIDA_SEED_EN) {
    const en = BIENVENIDA_SEED_EN[es];
    if (en) mem[key("en", es)] = en;
  }
  for (const es in CALIFICAR_PLATAFORMA_SEED_EN) {
    const en = CALIFICAR_PLATAFORMA_SEED_EN[es];
    if (en) mem[key("en", es)] = en;
  }
  for (const es in ONBOARDING_INICIAL_SEED_EN) {
    const en = ONBOARDING_INICIAL_SEED_EN[es];
    if (en) mem[key("en", es)] = en;
  }
  for (const es in SIN_EMOJIS_SEED_EN) {
    const en = SIN_EMOJIS_SEED_EN[es];
    if (en) mem[key("en", es)] = en;
  }
